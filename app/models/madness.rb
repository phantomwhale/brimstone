class Madness < ApplicationRecord
  belongs_to :hero
  ...
  # Check if this is a known madness from the chart
  def from_chart?
    madness_key.present?
  end

  # Check if this madness has stat modifiers
  def has_modifiers?
  ...
  def set_defaults
    self.modifiers ||= {}
    self.permanent = false if permanent.nil?
  end
